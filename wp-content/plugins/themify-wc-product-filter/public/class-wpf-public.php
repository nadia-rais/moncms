...
class WPF_Public
{
	...
	private function __construct() {
		...
		if ( ! empty( $_GET['id'] ) && ! empty( $_GET['wpf'] ) ) {
			add_filter( 'woocommerce_shortcode_products_query', array( $this, 'products_shortcode_disable_cache' ) );
	...
	public function register_assets() {
		...
		wp_register_script( $this->plugin_name . 'ui-touch', $plugin_url . 'js/jquery.ui.touch-punch.min.js', array( 'jquery-ui-mouse' ), $this->version, true );
	 ...
	 *
	 * uses wpf_page URL parameter
	 *
	 * @since 1.2.8
	 */
	function pagination_numbered() {
		...
		$this->unnullify_template( 'loop/pagination.php' );
	 ...
	 * @since 1.2.8
	 */
	public function pagination_load_more() {
		remove_action( 'woocommerce_after_shop_loop', array( $this, 'pagination_load_more' ), 100 );
	...
	public function parse_query( array $post, array $form, $build = true ) {
		...
		if ( $build ) {
			...
			if ( !empty( $data['pagination'] ) ) {
				...
				$this->append = false;
			} elseif ( isset( $data['pagination_type'] ) ) {
				remove_action( 'woocommerce_after_shop_loop', 'woocommerce_pagination', 10 );

				// pagination links are added here
				if ( $data['pagination_type'] !== 'pagination' ) {
					add_action( 'woocommerce_after_shop_loop', array( $this, 'pagination_load_more' ), 100 );
				} else {
					add_action( 'woocommerce_after_shop_loop', array( $this, 'pagination_numbered' ), 100 );
				}

				$this->nullify_template( 'loop/pagination.php' );
			}else if(!has_filter( 'woocommerce_after_shop_loop', 'woocommerce_pagination' )){
				// Re-add WC pagination for Themify themes that their pagination is set to infinity load in Theme setting
		...
		foreach ( $layout as $type => $item ) {
			if ( $type !== 'submit' ) {
				$key = 'wpf_' . WPF_Utils::strtolower( WPF_Utils::get_field_name( $item, $type ) );
				$key = urldecode( $key );
				...
				if ( !empty( $post[ $key ] ) || ( isset( $post[ $key ] ) && $post[ $key ] === '0' ) ) {
					...
					if ( $type === 'price' ) {
						if ( empty( $item['price_type'] ) || $item['price_type'] === 'slider' ) {
							$key2 = 'wpf_' . WPF_Utils::strtolower( WPF_Utils::get_field_name( $item, $type ) );
	...
	function change_query( $query ) {
		if ( $form = $this->get_form( sanitize_key( $_GET['id'] ) ) ) {
	...
	/**
	 * Adds a bogus random parameter to the [products] shortcode and prevent it from caching the posts.
	 * This is required for themes that use [products] to create the Shop page, eg. Divi.
	 *
	 * @return array
	 */
	function products_shortcode_disable_cache( $query ) {
