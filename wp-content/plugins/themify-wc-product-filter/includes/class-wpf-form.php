...
class WPF_Form {
    ...
    private function add_fields($data = array()) {
        ...
        <div class="wpf_lightbox_row ">
            ...
            <div class="wpf_lightbox_input">
                <input id="wpf_no_found_message" class="wpf_towidth" type="text" value="<?php echo ! empty( $data['no_found_message'] ) ? $data['no_found_message'] : __( 'No products were found matching your selection.', 'wpf' ) ?>" name="no_found_message" />
            </div>
        </div>
        <?php
        wp_reset_postdata();
    ...
    public function save_themplate(array $post) {
        ...
        if (!empty($post['layout'])) {
            ...
            $_keys = array( 'name', 'empty', 'group','reset_button', 'type', 'page', 'sort', 'pagination', 'posts_per_page', 'result', 'out_of_stock', 'scroll', 'result_type', 'tax_relation', 'pagination_type', 'no_found_message', 'infinitybuffer' );
            foreach ($_keys as $k) {
                if (!empty($post[$k])) {
    ...
    public function public_themplate(array $template, $page_id, array $request = array()) {
		...
		$no_found_message = ! empty( $template['data']['no_found_message'] ) ? $template['data']['no_found_message'] : __( 'No products were found matching your selection.', 'wpf' );
        ...
        <form
			...
			action="<?php echo add_query_arg( array( 'wpf_page' => 1 ) ); ?>"
			method="get"
        ...
        switch ($type):
            ...
            case 'sku':
                ...
                <div class="wpf_autocomplete">
                    <input type="text" autocomplete="off" name="wpf_<?php echo WPF_Utils::strtolower(WPF_Utils::get_field_name($args, $type)); ?>" value="<?php echo $value ? esc_attr($value) : '' ?>" />
                ...
                ?>  <?php if ( $show ) : ?>
                    <div class="wpf_<?php echo $type ?>_wrapp">
                        <input 
							...
							name="wpf_<?php echo WPF_Utils::strtolower(WPF_Utils::get_field_name($args, $type)); ?>"
							value="1"
						/>
                    </div>
                <?php endif; ?>
            ...
            case 'price':
                ...
                $name = 'wpf_' . WPF_Utils::strtolower(WPF_Utils::get_field_name($args, $type));
            ...
            default:
                ...
                <?php if (!empty($cats)): ?>
                    <?php if ($is_dropdown): ?>
                        <select  name="wpf_<?php echo WPF_Utils::strtolower(WPF_Utils::get_field_name($args, $type)); ?><?php if ('multiselect' === $args['show_as']): ?>[]<?php endif; ?>" <?php if ('multiselect' === $args['show_as']): ?>multiple="multiple" <?php if ($selected && ($type === 'wpf_cat' || $type === 'wpf_tag')): ?>data-selected="<?php echo $selected ?>" <?php endif; ?><?php endif; ?>class="wpf_dropdown">
                        <?php else: ?>
