  ...
  window.et_pb_init_modules = function() {
    ...
    $(document).ready(() => {
      ...
      if ($et_contact_container.length) {
        $et_contact_container.each(function() {
          ...
          $et_contact_form.submit(function(event) {
            ...
            etCore.api.spam.recaptcha.interaction(`Divi/Module/ContactForm/${form_unique_id}`).then(token => {
              et_message = '<ul>';
      ...
      window.et_pb_submit_newsletter = function($submit, event) {
        ...
        etCore.api.spam.recaptcha.interaction(`Divi/Module/EmailOptin/List/${list_id}`).then(token => {
          $.ajax({
            type: 'POST',
